/* ============================================================
   CATÁLOGO — COLCHONES FIRMEZA MEDIA (medio.html)
   Archivo propio de esta página (no se comparte con firme.js/suave.js).
   ...
   <div class="fila-productos" id="medioGrid">...</div> en medio.html
...
(function () {
  ...
  const CATEGORY_PREFIX = 'medio';

  function initMedioGrid() {
    const grid = document.getElementById('medioGrid');
    if (!grid) {
      console.warn('[medio] Falta #medioGrid en el HTML.');
      return;
    }
    ...
    nodes.forEach(node => {
      ...
      if (!rawId) {
        console.warn('[medio] Un producto no tiene data-id, se omite:', node);
        return;
      }
      ...
      if (node.dataset.sizes) {
        try {
          ...
          console.warn(`[medio] data-sizes inválido en "${rawId}" (revisa que sea JSON válido):`, err);
        }
      }
      ...
      if (node.dataset.gallery) {
        try {
          ...
          console.warn(`[medio] data-gallery inválido en "${rawId}" (revisa que sea JSON válido):`, err);
        }
      }
    ...
    if (registered.length === 0) {
      grid.innerHTML = '<p>No hay colchones de firmeza media cargados todavía.</p>';
      return;
    }
    ...
    grid.addEventListener('click', e => {
      ...
      if (typeof window.addToCart === 'function') {
        ...
        console.warn('[medio] window.addToCart no está definido.');
      }
  ...
  try {
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', initMedioGrid);
    } else {
      initMedioGrid();
    }
  } catch (err) {
    console.warn('[medio] No se pudo inicializar:', err);
  }
