/* ============================================================
   CATÁLOGO — COLCHONES FIRMES (firme.html)
   Archivo propio de esta página (no se comparte con medio.js/suave.js).
   ...
   <div class="fila-productos" id="firmeGrid">...</div> en firme.html
...
(function () {
  ...
  const CATEGORY_PREFIX = 'firme';

  function initFirmeGrid() {
    const grid = document.getElementById('firmeGrid');
    if (!grid) {
      console.warn('[firme] Falta #firmeGrid en el HTML.');
      return;
    }
    ...
    nodes.forEach(node => {
      ...
      if (!rawId) {
        console.warn('[firme] Un producto no tiene data-id, se omite:', node);
        return;
      }
    ...
    if (registered.length === 0) {
      grid.innerHTML = '<p>No hay colchones firmes cargados todavía.</p>';
      return;
    }
    ...
    grid.addEventListener('click', e => {
      ...
      if (typeof window.addToCart === 'function') {
        ...
        console.warn('[firme] window.addToCart no está definido.');
      }
  ...
  try {
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', initFirmeGrid);
    } else {
      initFirmeGrid();
    }
  } catch (err) {
    console.warn('[firme] No se pudo inicializar:', err);
  }
